#!/usr/bin/env sh
# Universal installer script for sx0 (https://sx0.ai)
#
# Usage:
...
if [ -z "$TARGET_DIR" ]; then
  if command -v sudo >/dev/null 2>&1 && [ -d "/usr/local/bin" ]; then
    ...
    TARGET_DIR="${TMPDIR:-/tmp}/sx0/bin"
    mkdir -p "$TARGET_DIR" 2>/dev/null || true
  fi
fi

TARGET_BIN="${TARGET_DIR}/sx0${BINARY_EXT}"
...
if [ -f "$TARGET_BIN" ] || [ -L "$TARGET_BIN" ]; then
  ...
  if [ -n "$CURRENT_VER" ]; then
    log_step "Found existing sx0 v${CURRENT_VER} at ${BOLD}${TARGET_BIN}${RESET}"
    ACTION="Updated"
  else
...
BINARY_NAME="sx0-${VERSION}-${OS}-${ARCH}${BINARY_EXT}"
...
TMP_DIR="$(mktemp -d 2>/dev/null || mktemp -d -t 'sx0install')"
trap 'rm -rf "$TMP_DIR"' EXIT

log_step "Downloading ${BINARY_NAME}..."
TMP_FILE="${TMP_DIR}/sx0"
...
if [ "$ACTION" = "Updated" ]; then
  log_info "sx0 successfully updated (v${CURRENT_VER} -> v${VERSION})!"
else
  log_info "sx0 v${VERSION} installed successfully!"
fi
...
printf "  ${AMBER}sx0 .${RESET}                 # inspect current directory\n"
printf "  ${AMBER}sx0 /path/to/project${RESET}  # or pass any directory path\n\n"
