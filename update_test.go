...
func TestFetchLatestRelease(t *testing.T) {
	fakeRelease := githubRelease{
		...
		Assets: []struct {
			...
			{
				Name:               "sx0-0.2.0-linux-amd64",
				BrowserDownloadURL: "https://example.com/download/sx0",
			},
		},
	}
	...
	t.Setenv("SX0_UPDATE_URL", server.URL)
	...
	if len(rel.Assets) != 1 || rel.Assets[0].Name != "sx0-0.2.0-linux-amd64" {
...
func TestUpdateStatePersistence(t *testing.T) {
	...
	statePath := filepath.Join(tmpDir, "sx0", "update_check.json")
...
func TestDownloadVerifiedAsset(t *testing.T) {
	assetName := "sx0-0.2.0-linux-amd64"
