...
Most file mutations (such as `git checkout`, `git pull`, branch switching, code generation, or edits from an external IDE) occur on the host filesystem outside of sx0's process boundary. Edits sx0 dispatches to a coding harness reuse the same reload path once they finish (see [Harness Editing & Agent Dispatch](agent-editing.md)).

A reload keeps each tab in the view it was in. Tabs with unsaved in-browser edits (`d.dirty`) are skipped so a refresh cannot wipe a buffer that has not been saved.
...
3. If an open file was edited or truncated on disk, sx0 showed stale cached lines. If the file shrank, attempting to scroll or jump to previous line numbers resulted in blank lines or out-of-bounds errors.
...
Instead of fetching entire files, sx0 leverages its windowed virtualization architecture (`CHUNK = 1000` lines):
...
- sx0 calculates the precise 1000-line chunk boundary enclosing that anchor:
...
Once all tabs have been updated in memory, sx0 updates the view surface:
...
- **Solution**: sx0 looks up the index dynamically using the object reference captured before the request:
  ```javascript
  const idx = S.tabs.indexOf(tgt.oldDoc);
  if (idx < 0) continue;
  ```
