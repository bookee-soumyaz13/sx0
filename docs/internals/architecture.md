...
| `/api/save`           | `POST` | Write a text file from the browser (`localPost`; disabled by `-no-edit`) | JSON (`{ok, path, size, mtime}`)           |
