...
This document details the architecture, lifecycle management, and security boundaries of sx0's Language Server Protocol subsystem ([`lsp.go`](../../lsp.go), [`lspnav.go`](../../lspnav.go), [`lspservers.go`](../../lspservers.go), [`lspsetup.go`](../../lspsetup.go), and [`calls.go`](../../calls.go)).
...
sx0 adopts a strictly Zero-Cost Lazy Architecture:
...
1. Zero Boot Overhead: During startup, sx0 does not launch any language servers. It scans `$PATH` concurrently via `exec.LookPath` to identify which server binaries exist on the host system.
...
Allowing arbitrary filesystem reads would introduce path traversal vulnerabilities. sx0 solves this with an In-Memory Target Allowlist:

```mermaid
sequenceDiagram
    autonumber
    participant UI as Browser Client
    participant Server as sx0 /api/lsp/def
...
sx0 provides full incoming and outgoing call hierarchy navigation (`Calls` tab in the right inspector) without holding complex graph state in server memory.
...
- Zero Server State: sx0 maintains no in-memory graph trees; cost scales strictly with the nodes the user expands.
