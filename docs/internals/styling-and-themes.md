...
This document describes sx0's styling architecture, CSS custom property design system, dynamic theme loading pipeline, and token specifications.

sx0 reads every colour in the user interface through a CSS custom property, known as a token. A theme is defined in a single CSS file that assigns values to these tokens.
...
| GitHub Dark      | `github-dark`      | dark   | GitHub dark default (default sx0 theme)|
...
1. Preview live without recompiling Go code by running sx0 in dev mode:
  ```bash
  go run . -dev . .
  ```
1. Run unit tests to verify theme conformance:
  ...
  go test ./...
  ```
  `TestThemesStylesheetJoinsEveryThemeFile` in [`sx0_test.go`](../../sx0_test.go) ensures all required tokens are present and selector IDs match filenames.
