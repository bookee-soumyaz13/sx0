...
This document details the architecture and performance strategies of sx0's windowed syntax highlighting engine ([`highlight.go`](../../highlight.go)).

## 1. The Syntax Highlighting Bottleneck

sx0 utilizes [Chroma](https://github.com/alecthomas/chroma) (a pure Go syntax highlighter modeled after Pygments) to tokenize ~280 programming languages without external dependencies or CGO.
...
To deliver instantaneous file opening (under 5 milliseconds), sx0 implements Viewport-Based Windowed Highlighting with Dual-Tier Background Refinement.

## 2. Windowed Highlighting Architecture

Rather than tokenizing the entire file, sx0 tokenizes only the slice of lines needed by the user's current scroll viewport, padded with leading and trailing context:
...
sx0 handles this with a Dual-Tier Processing Strategy:
...
sx0 maps token types to minimal 1-to-2 character CSS classes:
