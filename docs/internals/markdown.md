...
This document describes how sx0 renders Markdown files: the server-side conversion, the browser-side sanitizer, and the logic that keeps the rendered view and the source view in step.
...
The preference persists in `localStorage` under `sx0.mdPreview` and is restored in `boot()` in [`web/src/main.js`](../../web/src/main.js).
...
1. Strip every attribute from kept elements, then restore only those that follow these rules:
  ...
  - Class tokens only when they are `md-code`, start with `footnote`, or are highlighter tokens on `<i>`. Content cannot borrow sx0's layout classes such as `row`.
...
`mdLocal` resolves a scheme-less reference with `new URL(ref, base)`, where `base` is the stand-in origin `http://sx0.invalid/` plus the Markdown file's directory, each segment percent-encoded. A leading `/` resolves to the workspace root, as on GitHub. If the result lands on any other origin, the reference was not relative after all and gets no URL.
...
`mdEnhance` runs after sanitization and adds markup that sx0 itself creates:
...
Navigation in sx0 is line-based: the outline, go to line, search results, references, history, and `openFile(path, { line })`. All of them call `centerLine(n)` in [`web/src/tabs.js`](../../web/src/tabs.js), which hands off to `previewLine(n)` while previewing.
