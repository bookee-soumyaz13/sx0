...
This document describes the design and implementation of sx0's editing flow:
...
Harnesses are discovered automatically, the same way language servers are. Editing becomes available as soon as sx0 finds one installed, but nothing ever runs until the user picks one, and that choice is remembered between runs. `-no-agent` removes the feature entirely; `-agent` pins a harness for scripted use and takes the choice away from the UI.

## 1. The Dispatcher Model

sx0 does not author changes. No endpoint accepts file content; it composes a prompt and reloads whatever the harness wrote.
...
2. sx0 composes a prompt from that instruction plus the referenced source.
3. sx0 spawns a coding harness already installed on the machine, with the workspace as its working directory.
4. The harness makes the change.
5. sx0 works out what moved and reloads it in place.
...
sequenceDiagram
    ...
    participant S as sx0 server (agent.go)
...
```
$XDG_CONFIG_HOME/sx0/settings.json     # when XDG_CONFIG_HOME is set
...
This follows `stateFilePath` in [`update.go`](../../update.go) and sits beside the anonymous ID written by [`telemetry.go`](../../telemetry.go). sx0 never writes its own state into a working tree: there is no `.sx0/` directory in the repository.
...
By default, sx0 uses the least capable (fastest and cheapest) model from each harness's available model list, while letting users choose any available model from the harness menu or picker.

A full command template is accepted anywhere a harness name is, and must contain `{prompt}`:

```bash
sx0 -agent "claude -p --permission-mode acceptEdits {prompt}"
```

The template is split on whitespace, and `{prompt}` is substituted inside each token, so both `{prompt}` and `--prompt={prompt}` work. Presets are a convenience, not a coupling: because a template is always available, a harness that changes its flags is a one-line fix by the user rather than a sx0 release.
...
As the harness runs, lines from stdout and stderr are streamed in real time to the terminal stdout where sx0 is running (`lineStreamer`), prefixed with `[<harness>]` so the developer can see exactly what the model is thinking, doing, and editing as it happens.
...
sx0 dispatched the harness, so it knows when the work ended. Completion is detected by the process exiting, not by watching the filesystem. There is no `fsnotify` dependency, no polling of the tree, and the single-binary, zero-dependency footprint is unchanged.
...
The edit endpoint runs a general-purpose coding agent with shell access as the user who started sx0. `localPost` restricts it to sx0's own page reached by IP address or `localhost`. That shuts out other websites and DNS rebinding, and makes editing unavailable through the hostname-based tunnels and reverse proxies described in the README. It is not authentication: with `-host 0.0.0.0`, anyone who can reach sx0 by IP, for example over Tailscale, can dispatch an edit. Exposing editing beyond a trusted network requires an authentication story sx0 does not yet have.

The explicit first-run pick matters for the same reason. Auto-enabling on discovery would mean any sx0 instance on a machine with a harness installed is a code execution endpoint that nobody opted into.
