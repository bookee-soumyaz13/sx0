...
// run a known installer for it, and pick the result up without restarting sx0.
...
type lspInstall struct {
	...
	Auto bool     // sx0 may run it: user-level and non-interactive. Otherwise only shown.
}
...
// registryFor lists every server sx0 knows for rel's extension, best first,
// installed or not.
...
// MissingLang names the language of rel when sx0 knows servers for it but none
...
// installed while sx0 is running is used on the next request.
...
func (m *lspManager) Install(name string, option int) (*lspJob, error) {
	...
	in := opts[option]
	if !in.Auto {
		return nil, fmt.Errorf("sx0 does not run %q; run it in a terminal", strings.Join(in.Cmd, " "))
	}
...
// from sx0's own page. Browsers send Origin on every POST, so a page from another
...
func localPost(w http.ResponseWriter, r *http.Request) bool {
	...
	if host != "localhost" && net.ParseIP(host) == nil {
		fail(w, http.StatusForbidden, "open sx0 by IP address or localhost to change files")
		return false
	}
	if o, err := url.Parse(r.Header.Get("Origin")); err != nil || o.Host != r.Host {
		fail(w, http.StatusForbidden, "request did not come from sx0")
		return false
	}
	return true
}
