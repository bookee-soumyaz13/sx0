...
func fetchLatestRelease(repo string) (*githubRelease, error) {
	...
	req.Header.Set("User-Agent", "sx0-updater")
...
// It ensures that checking for updates never blocks sx0 startup (<1ms).
...
func printUpdateNotification(latestVer, currentVersion string) {
	msg := fmt.Sprintf("a new version of sx0 (v%s) is available (current: v%s)", latestVer, currentVersion)
	uiStatus("step", msg, "run 'sx0 --update' to upgrade", 0, os.Stderr)
}

// runSelfUpdate implements sx0 --update.
func runSelfUpdate(currentVer string) error {
	...
	if compareSemver(latestVer, currentVer) <= 0 {
		uiStatus("ok", fmt.Sprintf("sx0 is already up to date (v%s)", currentVer), "", 0, os.Stdout)
		return nil
	}
	...
	expectedAsset := fmt.Sprintf("sx0-%s-%s-%s%s", latestVer, runtime.GOOS, runtime.GOARCH, ext)
	...
	tmpFile, err := os.CreateTemp(dir, "sx0-update-*")
	if err != nil {
		// If directory is not writable, warn user about permissions
		if os.IsPermission(err) {
			return fmt.Errorf("permission denied writing to %s. Try running with 'sudo sx0 --update'", dir)
		}
		// Try temp directory as fallback
		tmpFile, err = os.CreateTemp("", "sx0-update-*")
		if err != nil {
	...
	if runtime.GOOS == "windows" {
		...
		if err := os.Rename(tmpPath, execPath); err != nil {
			...
			if err := copyOrMove(tmpPath, execPath); err != nil {
				if os.IsPermission(err) {
					return fmt.Errorf("permission denied replacing %s. Try running with 'sudo sx0 --update'", execPath)
				}
	...
	uiStatus("ok", fmt.Sprintf("sx0 successfully updated to v%s at %s", latestVer, execPath), "", 0, os.Stdout)
	return nil
}
...
func copyOrMove(src, dst string) error {
	...
	tmp, err := os.CreateTemp(dir, "sx0-replace-*")
	if err != nil {
		return err
	}
	tmpName := tmp.Name()
