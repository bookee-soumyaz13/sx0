...
func TestAgentSpecResolution(t *testing.T) {
	...
	if _, err := newAgentManager(root, "sx0-not-a-real-binary {prompt}", nil); err == nil {
...
func TestAgentSelectPersistsOutsideWorkspace(t *testing.T) {
	...
	if err := m.Select("sx0-not-a-real-binary"); err == nil {
	...
	if _, err := os.Stat(filepath.Join(cfg, "sx0", "settings.json")); err != nil {
...
// Outside a repository sx0 cannot name what a harness touched. The job must say
