...
// is required for sx0 to work; a server is used only if its binary is found on
...
// from PATH, so a server installed from sx0, or by hand after sx0 started, is
// found without restarting the shell sx0 was launched from.
...
func (m *lspManager) State(rel string) (lspState, string) {
	...
	if def == nil {
		...
		// file type sx0 knows may still get a server, so keep the UI asking.
