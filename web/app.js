...
function paint() {
  ...
  const plain = !!(d.dirty && d.text);
  for (let i = first; i < last; i++) {
    const n = i + 1;
    const body = plain ? esc(d.text[i] ?? '') : (d.lines[i] === undefined ? '' : d.lines[i]);
    let rc = 'row', gc = 'g';
    if (n === d.cur) rc += ' cur';
...
function lineEndCol(d) {
  if (d.text) return (d.text[d.cur - 1] || '').length;
  return 0;
}
...
function updateStatus() {
  ...
  if (d && d.isImage) {
    ...
    if (posEl) {
      if (d) {
        const col = d.col === Infinity ? lineEndCol(d) + 1 : (d.col || 0) + 1;
        posEl.textContent = (d.dirty ? '• ' : '') + 'Ln ' + d.cur + ', Col ' + col;
      } else {
        posEl.textContent = '';
      }
    }
  ...
  if (saveBtn) {
    saveBtn.hidden = S.meta?.edit === false;
    saveBtn.classList.toggle('dirty', !!(d && d.dirty));
  }
...
function closeTab(i) {
  const doomed = S.tabs[i];
  if (doomed?.dirty && !confirm('Discard unsaved changes in ' + doomed.name + '?')) return;
  clearSelectAll();
...
function drawTabs() {
  $('#tabs').innerHTML = S.tabs.map((t, i) =>
    '<div class="tab' + (i === S.active ? ' active' : '') + (t.isImage ? ' tab-image' : '') + (t.diffAvailable ? ' git-modified' : '') + (t.dirty ? ' dirty' : '') + '" data-i="' + i + '" title="' + esc(t.path) + (t.dirty ? ' — unsaved' : '') + '">' +
    ...
    (t.dirty ? '<span class="tab-dirty-dot" title="Unsaved changes">●</span>' : '') +
...
async function ensureBuffer(d = doc_()) {
...
function insertText(str, op = 'type') {
  const d = doc_();
...
let vimMode = 'NORMAL'; // 'NORMAL' | 'VISUAL' | 'VISUAL_LINE' | 'INSERT'
let vimCount = '';
let vimPending = '';
...
function updateVimStatus() {
  ...
  if (vimMode === 'VISUAL_LINE') {
    ...
    chip.classList.add('mode-insert');
    modeLabel = 'INSERT';
  } else {
...
function enterVimInsert(where = 'i') {
  const d = doc_();
  if (!d || d.isImage) return;
  (async () => {
    if (!(await ensureBuffer(d))) return;
    const len = lineLen(d, d.cur);
    const col = d.col === Infinity ? len : (d.col || 0);
    if (where === 'a') d.col = Math.min(len, col + 1);
    else if (where === 'A') d.col = len;
    else if (where === 'I') {
      const text = d.text[d.cur - 1] || '';
...
const VIM_SHORTCUT_SECTIONS = [
  ...
  },
  {
    title: 'Insert & Edit',
    items: [
      [['i', 'a', 'I', 'A'], 'Insert at cursor / after cursor / first non-blank / end of line'],
      [['o', 'O'], 'Open a line below / above and insert'],
      [['Esc'], 'Return to Normal mode'],
      [['x', 'dd'], 'Delete character / delete line'],
      [['d'], 'In Visual mode: delete the selection'],
    ]
  },
  {
...
const SHORTCUTS = [
  ...
  [['Alt+E'], 'Edit selection with coding agent'],
  [['Mod+S'], 'Save file'],
...
function initShortcuts() {
  ...
  addEventListener('keydown', e => {
    ...
    if (mod && !e.shiftKey && !e.altKey && (e.key === 's' || e.key === 'S')) {
      if (editingEnabled() && !inField(document.activeElement)) {
        e.preventDefault();
        saveActive();
        return;
      }
    ...
    if (!mod && (e.key === 'ArrowDown' || (!editingEnabled() && e.key === 'j'))) { e.preventDefault(); moveCursor(1, shift); return; }
    if (!mod && (e.key === 'ArrowUp' || (!editingEnabled() && e.key === 'k'))) { e.preventDefault(); moveCursor(-1, shift); return; }
...
const COMMANDS = [
  ...
  { name: withKeys('Save File ({Mod+S})'), run: () => saveActive() },
