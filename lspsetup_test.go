...
// Only a POST from sx0's own page, addressed by IP or localhost, may install.
...
func TestLookPathInExtraDir(t *testing.T) {
	dir := t.TempDir()
	name := "sx0-test-fake-language-server"
	file := name
...
// Every recipe is well formed, and nothing sx0 runs by itself asks for root.
...
func TestInstallRefusals(t *testing.T) {
	...
	if _, err := m.Install("sx0-no-such-server", 0); err == nil {
